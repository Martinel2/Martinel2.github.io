...
def portfolio():
    ...
    for i, c in enumerate(DATA['cases'], 1):
        if c['project'] not in seen:
            ...
            my_work = '<ul class="me-work">' + ''.join(f'<li><b>{escape(title)}</b><span>{escape(description)}</span></li>' for title, description in project['contributions']) + '</ul>'
            team = ''.join(f'<div class="me"><dt><span class="me-badge">MY ROLE</span>{escape(role.replace(" (본인)", "").replace(" (me)", ""))}</dt><dd><p class="me-summary">{escape(work)}</p>{my_work}</dd></div>' if is_me(role) else f'<div><dt>{escape(role)}</dt><dd>{escape(work)}</dd></div>' for role, work in project['team'])
...
<p class="profile-summary">{escape(DATA['overview']['profileLine1'])}<br>{escape(DATA['overview']['profileLine2'])}</p><div class="pdf-buttons"><a class="button pdf-button" href="resume.pdf" target="_blank" rel="noopener"><img src="assets/icons/file-text.svg" alt="" width="18" height="18">이력서 PDF</a><a class="button pdf-button" href="portfolio.pdf" target="_blank" rel="noopener"><img src="assets/icons/briefcase.svg" alt="" width="18" height="18">포트폴리오 PDF</a></div></aside></section>
<div id="work" class="work-anchor"></div>
<div class="work-layout"><aside class="toc"><div class="toc-inner"><p class="eyebrow">목차 <span>{len(DATA['cases']):02d}</span></p><nav aria-label="프로젝트 목차">{contents}<p class="toc-group">MORE</p><a class="toc-extra" href="#beyond"><span>{len(DATA["cases"]) + 1:02d}</span><span>코드 밖에서도 이어지는 경험</span><span class="toc-arrow">↗</span></a><a class="toc-extra" href="#writings"><span>{len(DATA["cases"]) + 2:02d}</span><span>{home_text("sections", "writingsTitle")}</span><span class="toc-arrow">↗</span></a></nav><div class="toc-foot pdf-buttons"><a class="button pdf-button" href="resume.pdf" target="_blank" rel="noopener"><img src="assets/icons/file-text.svg" alt="" width="18" height="18">이력서 PDF</a><a class="button pdf-button" href="portfolio.pdf" target="_blank" rel="noopener"><img src="assets/icons/briefcase.svg" alt="" width="18" height="18">포트폴리오 PDF</a></div></div></aside><div class="cases">{''.join(project_sections)}</div></div>
<section class="more-work" id="beyond"><span class="eyebrow">BEYOND THE PROJECTS</span><h2>코드 밖에서도 이어지는 경험</h2><div class="more-grid">
