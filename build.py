...
def portfolio():
    ...
    for i, c in enumerate(DATA['cases'], 1):
        if c['project'] not in seen:
            ...
            team = ''.join(f'<div class="me"><dt><span class="me-badge">ME</span>{escape(role.replace(" (본인)", "").replace(" (me)", ""))}</dt><dd><p class="me-summary">{escape(work)}</p>{my_work}</dd></div>' if is_me(role) else f'<div><dt>{escape(role)}</dt><dd>{escape(work)}</dd></div>' for role, work in project['team'])
