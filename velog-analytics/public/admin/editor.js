...
const labels = {dialogTitle:'이력서 선택창 제목',pdfLabel:'PDF 보기 버튼 이름',docxLabel:'Word 다운로드 버튼 이름',hero:'인사말',greeting:'인사말 첫 줄',nameLine:'이름과 인사',about:'소개',actions:'소개 아래 버튼',resume:'이력서 버튼',portfolio:'포트폴리오 버튼',linkedin:'LinkedIn 버튼',github:'GitHub 버튼',skillsEyebrow:'기술 영문 표기',projectsEyebrow:'프로젝트 영문 표기',projectsTitle:'프로젝트 섹션 제목',writingsEyebrow:'회고록 영문 표기',writingsTitle:'회고록 제목',writingsLinkLabel:'블로그 전체 링크 이름',writingsUrl:'블로그 전체 주소',homeLabel:'홈 탭',aboutLabel:'소개 탭',skillsLabel:'기술 탭',projectsLabel:'프로젝트 탭',activitiesLabel:'활동 탭',affiliationLabel:'소속 항목 이름',roleLabel:'역할 항목 이름',detailsLabel:'프로젝트 팝업 열기 문구',updated:'최종 수정일',overview:'첫 화면',title:'제목',eyebrow:'상단 설명',description:'소개',experience1Title:'주요 경험 1 · 제목',experience1Description:'주요 경험 1 · 설명',experience2Title:'주요 경험 2 · 제목',experience2Description:'주요 경험 2 · 설명',experience3Title:'주요 경험 3 · 제목',experience3Description:'주요 경험 3 · 설명',profileRole:'직무',profileLine1:'프로필 소개 · 첫 줄',profileLine2:'프로필 소개 · 둘째 줄',label:'구분 / 링크 이름',short:'목차 이름',category:'분야',period:'기간',role:'역할',summary:'요약',tags:'기술 태그',problem:'문제 상황',ownership:'내가 맡은 부분',processTitle:'과정 섹션 제목',experiments:'진행 과정',designSteps:'판단과 구현 과정',judgment:'판단',verification:'확인한 결과',options:'해결 방안 비교',decision:'선택 이유',implementation:'구현 내용',diagram:'구조도 · Mermaid 원문',diagramCaption:'구조도 설명',resultTitle:'결과 제목',result:'결과',metrics:'성과 수치',comparison:'결과 비교표',columns:'열 제목',rows:'비교 내용',note:'비교 조건 / 주의점',tradeoff:'결과 범위와 한계',next:'다음 검증',source:'근거',links:'관련 링크',url:'링크 주소',href:'페이지 내 연결',contributions:'내 주요 개발',origin:'시작 계기',goal:'풀고 싶었던 문제',myRole:'내 역할',team:'팀 구성',cover:'프로젝트 표지',gallery:'프로젝트 사진',image:'사례 사진',alt:'이미지 대체 설명',caption:'사진 설명',fields:'본문',writings:'블로그 글'};
const resumeLabels = {text131:'이력서 Word 다운로드 이름',text1:'이력서 상단 표기',text2:'소개 문구 앞부분',text3:'소개 문구 뒷부분',text4:'이력서 직무 표기',text5:'어떤 개발자인가요? · 첫 문단',text6:'어떤 개발자인가요? · 둘째 문단',text19:'섹션 제목',text20:'섹션 영문 표기',text21:'프로젝트 이름',text22:'진행 기간',text23:'프로젝트 소개',text24:'담당 역할',text25:'팀원과의 역할 구분',text26:'PDF 변환 · 문제 / 해결 / 성과',text28:'문서 병렬 처리 · 문제 / 해결 / 성과',text27:'근거 검색 · 문제 / 해결 / 성과',text29:'사용자 정의 작업 · 문제 / 해결 / 성과',text30:'Jev 라우팅 · 문제 / 해결 / 성과',text35:'서비스 설계 기여',text36:'평가 수치의 기준',text37:'PDF 변환 상세 링크',text39:'문서 병렬 처리 상세 링크',text40:'Agent 상세 링크',text43:'근거 검색 상세 링크',text44:'프로젝트 이름',text45:'진행 기간',text46:'프로젝트 소개',text47:'담당 역할',text112:'팀원과의 역할 구분',text50:'데이터 정제 · 문제 / 해결',text51:'데이터 정제 · 성과',text54:'비용 산정 기준',text52:'복약 챗봇 · 문제 / 해결 / 성과',text48:'의약품 검색과 위험정보 기능',text49:'복약 관리와 가족 기능',text53:'팀 협업과 문서화',text55:'데이터 정제 상세 링크',text56:'복약 챗봇 상세 링크',text130:'멘토 공로상 수상 설명',text113:'인증·인가 항목 제목',text128:'쇼핑몰 GitHub 링크 이름',text129:'Todo GitHub 링크 이름'};
...
const matrixLabels = {experiments:['단계 제목','문제 상황','시도와 결과','판단'],options:['해결 방안','기대 효과','실험 결과','채택 여부와 이유'],metrics:['항목','수치','설명'],contributions:['개발 항목','내용'],team:['역할','담당 내용']};
let language="ko";
...
function refresh(){$('#content-language').disabled=busy;const intro=data&&(sections[active]?.path.join('.')==='home.hero'||(sections[active]?.kind==='resume'&&valueAt(sections[active].path).fields.text2!==undefined));$('#intro-preview').hidden=!intro;if(intro){const f=data.resume.find(r=>'text2' in r.fields).fields;$('#intro-preview').textContent=(data.home?.hero.greeting||'안녕하세요,')+'\n'+(data.home?.hero.nameLine||'김재형입니다.')+'\n\n'+f.text2+'\n'+f.text3;}const count=changed().length;$('#dirty').textContent=count?`${count}개 항목 수정됨 · 아직 저장하지 않았습니다.`:'저장된 내용과 같습니다.';$('#review').disabled=busy||!canSave||!count;$('#reload').disabled=busy||!data;$('#backup').disabled=busy||!data;$('#save').disabled=busy;$('#save').textContent=busy?'저장 중…':'저장하고 배포';$('#cancel').disabled=busy;document.querySelectorAll('#fields textarea,#menu button').forEach(n=>n.disabled=busy);}
...
function render(){sections=[];
 if(data.home)for(const [key,title,help] of [
  ['hero','홈 인사말','인사말과 이름, 상단 직무 문구를 수정합니다. 아래 개발자 소개 문구는 홈·이력서의 소개에서 수정하세요.'],
  ['about','홈 소개 제목·마지막 문단','소개 제목과 마지막 문단입니다. 앞의 두 문단은 홈·이력서의 소개에서 수정하세요.'],
  ['actions','이력서·포트폴리오·SNS 버튼','버튼 이름과 LinkedIn·GitHub 주소를 수정합니다. 이력서와 포트폴리오 버튼은 사이트 내부 페이지로 연결됩니다.'],
  ['navigation','상단 메뉴 이름','모든 페이지 상단에 표시되는 메뉴 이름입니다.'],
  ['sections','기술·프로젝트·회고록 제목','섹션 제목과 영문 표기, 블로그 전체 링크를 수정합니다. 기술 제목은 홈·이력서의 기술에서 수정하세요.'],
  ['projectLabels','프로젝트 카드 안내 문구','소속·역할 항목 이름과 팝업을 여는 문구입니다. 실제 소속과 역할은 프로젝트 팝업에서 수정하세요.']
 ])sections.push({group:'홈 화면',title,path:['home',key],help});
...
async function load(nextLanguage=language){if(data&&changed().length&&!confirm('저장하지 않은 변경을 버리고 다시 불러올까요?')){$('#content-language').value=language;return;}busy=true;refresh();status('최신 내용을 불러오는 중입니다.');try{
 const res=await fetch('/admin/api/content?lang='+nextLanguage,{cache:'no-store',signal:AbortSignal.timeout(35000)});if(res.status===401)throw Error('로그인이 만료됐습니다. 새로고침해 다시 로그인해 주세요.');const body=await res.json();if(!res.ok)throw Error(body.error);
 if(!body.data.overview||!body.data.resume)throw Error('관리자 편집용 사이트 업데이트를 기다리고 있습니다. 잠시 후 다시 불러와 주세요.');
 language=nextLanguage;$('#content-language').value=language;data=body.data;original=structuredClone(data);sha=body.sha;canSave=body.canSave;render();status(canSave?(language==='en'?'영문 내용을 불러왔습니다. 한국어 수정은 자동 번역되지 않습니다.':'한국어 내용을 불러왔습니다. 영문은 편집할 언어에서 별도로 수정하세요.'):'내용 편집과 백업은 가능합니다. 저장 기능은 아직 연결되지 않았습니다.');
 }catch(e){status(e.name==='TimeoutError'?'불러오기가 지연되고 있습니다. 잠시 후 다시 시도해 주세요.':e.message,true);}finally{$('#content-language').value=language;busy=false;refresh();$('#reload').disabled=false;}}
$('#reload').addEventListener('click',()=>load());
$('#content-language').addEventListener('change',event=>load(event.target.value));
$('#backup').addEventListener('click',()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)+'\n'],{type:'application/json'}));const a=el('a');a.href=url;a.download=`portfolio-content-${language}-backup.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});
