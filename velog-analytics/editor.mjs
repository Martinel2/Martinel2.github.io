...
async function github(env, method = 'GET', data, lang = 'ko') {
  if (method === 'GET' && !env.GITHUB_TOKEN) {
    // Public read remains available before the repository-scoped token is connected.
    const raw = await fetch('https://raw.githubusercontent.com/Martinel2/Martinel2.github.io/main/content'+(lang==='en'?'.en':'')+'.json', { signal: AbortSignal.timeout(15000), headers: { 'Cache-Control': 'no-cache' } });
  ...
  return fetch(endpoint.replace('content.json', lang==='en'?'content.en.json':'content.json') + (method === 'GET' ? '?ref=main' : ''), {
...
export async function editor(request, env) {
  ...
  if (!['ko', 'en'].includes(lang)) return json({ error: '지원하지 않는 언어입니다.' }, 400);
  ...
  try {
    const currentResponse = await github(env, 'GET', undefined, lang);
    ...
    let binary = ''; for (const b of bytes) binary += String.fromCharCode(b);
    const saved = await github(env, 'PUT', { message: `관리자 페이지에서 포트폴리오 ${lang} 내용 수정`, branch: 'main', sha: current.sha, content: btoa(binary) }, lang);
